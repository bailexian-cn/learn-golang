...
func main() {
	...
	fmt.Printf("%v", c1) // {{1 1} c d}
	fmt.Println()
	fmt.Printf("%#v", c1) // main.C{A:main.A{a:1, b:1}, c:"c", d:"d"}
}
```
...
上述例子中发现`cat.Name`鹤`catCopy.Name`的内存地址一致，即`指针属性`并没有被深拷贝。所以在需要进行深拷贝的情况，可以增加方法来完成深拷贝。比如
